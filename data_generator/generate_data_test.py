...
def test_make_gitlab_issue(gitlab_changes):
    ...
    assert isinstance(issue['object_attributes']['created_at'], datetime.datetime)
    assert isinstance(issue['object_attributes']['closed_at'], datetime.datetime)
...
def test_github_request(github_changes):
    ...
    assert len(request.headers['X-hub-signature']) == 45  # ex: 'sha1=78492d6d232fafa4c58b31888cd131d1db9f5dd5'
    assert request.headers['User-agent'] == 'GitHub-Hookshot/mock'
    assert request.headers['Content-type'] == 'application/json'
    assert request.headers['Mock'] is True
    assert len(request.data) >= 40  # post body should contain at least one commit SHA
