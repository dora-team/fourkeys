...
def prometheus_verification(signatures, body):
    """
    Verifies that the signature received from the prometheus event is accurate
    """
    ...
    bearer_token = str(signatures).split(" ")[1]

    if len(bearer_token) == 0:
        raise Exception("Prometheus alertmaneger bearer token is empty")

    try:
        # Get secret from Cloud Secret Manager (should be generated by terraform)
        expected_signature = get_secret(PROJECT_NAME, "fourkeys_prometheus_secret", "latest")

    except Exception as e:
        print(e)

    if expected_signature.decode() in bearer_token:
        return True
    else:
        return False
...
def get_source(headers):
    ...
    if "Alertmanager" in headers.get("User-Agent", ""):
        return "prometheus"
...
AUTHORIZED_SOURCES = {
    ...
    "prometheus": EventSource(
        "Authorization", prometheus_verification
        ),
}
