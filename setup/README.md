...
If Prometheus Alertmanager is enabled you have to add a new webhook_configs receiver on Alertmanager setting the credential generated by Terraform and available on `fourkeys_prometheus_secret` from Google Secret Manager.
...
- name: 'fourkeys'
  webhook_configs:
  - send_resolved: true
    url: <Event Handler endpoint>
    http_config:
      authorization:
        credentials: <Secret value from fourkeys_prometheus_secret created by terraform>
```
